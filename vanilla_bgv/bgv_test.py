...
def test_add_noise_free():
  ...
  ct1 = bgv.encrypt(
      pt1, pk=pk, params=ERROR_FREE_TEST_PARAMS, debug_data=debug_data
  )
  ct2 = bgv.encrypt(
      pt2, pk=pk, params=ERROR_FREE_TEST_PARAMS, debug_data=debug_data
  )
...
def test_iterated_add_noisy():
  ...
  expected = m1
  max_error = math.log2(
      params.ciphertext_coeff_modulus / (2 * params.plaintext_coeff_modulus)
      - 0.5
  )
